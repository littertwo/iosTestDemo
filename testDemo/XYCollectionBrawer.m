...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.testIamgeView.image = [UIImage imageNamed:model.url];
    return cell;
}

-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    XYImageModel *model = _dataArray[indexPath.item];
    if (model.isVideo && self.imageClickBlock) {
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (scrollView == self.imageViewCollection) {
        //图片相关
        NSInteger page = scrollView.contentOffset.x/SCREENSIZE.width;
        NSLog(@"滚动到：%zd",page);
        if (page == 0) {//滚动到左边
            scrollView.contentOffset = CGPointMake(SCREENSIZE.width * (_dataArray.count - 2), 0);
