...
//  testDemo
//
//  Created by XYCoder on 2017/4/30.
...
+(instancetype)sharedIntroudence{
    ...
    dispatch_once(&onceToken, ^{
        ...
        introduce.backgroundColor = [[UIColor blackColor]colorWithAlphaComponent:0.5f];
...
-(void)showIntroduceWithItems:(NSArray<XYIntroduceItemObj *> *)items{
    ...
    for (XYIntroduceItemObj *objc in items) {
        ...
        switch (quadrant) {
            case 1:
                imageRect = CGRectMake(self.frame.size.width/2.0f, CGRectGetMaxY(objc.rect), objc.rect.origin.x-self.frame.size.width/2.0f , 100);
                labelRect = CGRectMake(80, CGRectGetMaxY(imageRect) +10, self.frame.size.width - 160, 60);
                buttonRect = CGRectMake(self.frame.size.width - 160, CGRectGetMaxY(labelRect), 80, 25);
                break;
